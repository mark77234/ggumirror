...
**uniform scale 하나뿐이다.** 제스처는 오브젝트가 누운 방향으로 투영한 **스칼라 하나**
(`alongWidth`)를 넘기고, 높이는 원본 비율에서 다시 계산된다(`resized(width:canvas:)`).
폭과 높이를 따로 갱신하는 경로는 만들지 않는다.

- **rotation-aware**: 90° / 180° / 270° 어디에서도 "바깥으로 끌면 커진다"가 통하고 비율이 유지된다.
  화면 x/y를 width/height에 각각 넣는 방식은 금지.
- **캔버스를 안다**: 정규화 높이는 캔버스 비율에 달려 있다. 정사각 스티커 캔버스에서
  거울 비율(0.46)로 높이를 구하면 스티커가 납작해진다 — 그래서 `canvas`를 넘긴다.
- **두들은 정사각 기준으로 그린다**: 어떤 rect가 와도 짧은 변에 맞춰(aspect fit) 그리므로
  frame이 찌그러져 있어도 디자인이 늘어나지 않는다.
- **최대 상한 없음**: 제품 차원의 max scale / maxWidth / size 상한을 두지 않는다.
  최소 크기(`StickerObject.minimumWidth`)만 지킨다.
- **viewport 확대(1…4)와 오브젝트 크기는 별개다.** 보기 배율이 4배 한계라고
  오브젝트가 4배에서 멈추지 않는다.
- NaN / 무한 / 0은 방어한다. 합리적인 finite 값은 자르지 않는다.
- 캔버스보다 큰 오브젝트는 **캔버스 경계에서만 잘린다.** Editor · 미리보기 · 실제 거울 ·
  Capture · 스티커 최종 PNG가 같은 규칙을 쓴다.

텍스트 크기 조절은 별도 정책(`TextPolicy`)이고 이번 수정으로 바꾸지 않았다.
