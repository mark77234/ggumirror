...
## Mirror Controls (C-2B)

화면을 탭하면 나타나는 컨트롤은 넷이다. 자동 숨김 시스템은 하나 그대로다.
...
- 전환 / 플래시는 홈으로와 **같은 재질**(어두운 잉크 원)이다. 새 UI 언어를 만들지 않았다
- 플래시가 켜져 있으면 흰 배경 + 어두운 아이콘으로 반전된다 — 아이콘만 봐도 상태가 보인다
- symbol: `bolt.fill` / `bolt.slash.fill` /
  `arrow.trianglehead.2.clockwise.rotate.90.camera`.
  **이름을 추측하지 않는다** — 테스트가 SDK에 실제로 있는지 확인한다
- 접근성: "카메라 전환" · "플래시 켜기" / "플래시 끄기" · "촬영" · "홈으로"
- 촬영 중이거나 카메라가 준비되지 않았으면 전환은 비활성(40% 투명)이다
- 촬영 결과에는 컨트롤이 **하나도** 들어가지 않는다 — 화면 스냅샷을 쓰지 않는다

화면 플래시(전면 하드웨어 flash가 없는 기기)는 촬영 순간에만 흰 화면을 띄운다.
컨트롤 위에 보여도 되지만 **저장되는 사진에는 합성되지 않는다** — 조명이지 장식이 아니다.
...
상점 안에서만 나눈다. **새 Bottom Tab을 만들지 않는다.**
제목 아래 잉크 칩 두 개(거울 / 스티커)로 바꾸고, 선택은 잉크 채움으로만 표시한다.
시스템 세그먼트 컨트롤을 쓰지 않는다. 전환은 `InkMotion`(easeInOut 0.26초).

### 스티커 화면

위에서 아래로: `스티커 만들기` 카드 → **내 스티커**(2열) → **스티커 상점**.

- 내 스티커 카드 = 투명 미리보기 + 이름. 그 이상 정보를 얹지 않는다.
- 투명 PNG라 미리보기 뒤에만 아주 옅은 체크무늬를 깐다 — **최종 PNG는 그대로 투명하다.**
- 내 거울 갤러리와 같은 2열 리듬(`GalleryLayout.columns`)을 쓴다.
- 스티커 상점은 점선 테두리 + "아직 등록된 스티커가 없어요". 가짜 카드를 채우지 않는다.

### Sticker Picker (Mirror Editor)

`기본 스티커` / `내 스티커` 두 칸. 기본은 두들 42종이고 legacy 26종은 여전히 나오지 않는다.
내 스티커가 없으면 "아직 만든 스티커가 없어요" + 만들기.
Creator에서 저장하고 나오면 picker의 **내 스티커로 돌아오고 방금 만든 것이 맨 앞**에 있다.
자동으로 거울에 얹지 않는다 — 사용자가 눌러서 놓는다.

### 배치 = 불변 스냅샷

내 스티커를 거울에 놓을 때 지금 완성 PNG를 **사진 asset으로 복사**해 둔다.
거울은 라이브러리의 `finalAssetID`를 직접 참조하지 않는다.

새 `StickerSource` case를 만들지 않았다 — `.photo`가 이미 "id로 참조하는 불변 bitmap + 비율"이라
거울 저장 형식(schemaVersion 3) · GC · 렌더 · 크기 조절이 전부 그대로 동작한다.
덕분에 원본 수정 / 삭제와 거울의 수명이 자연스럽게 분리된다.
...
프레임은 **없앨 수 있다.** 상점 9번째 상품이 아니라 **프레임 편집 option**이다 —
자기 거울에서 프레임 색을 고르듯 "투명"을 고른다.

- 시트 이름은 "프레임 색", 첫 칸이 "투명"이다. 색 8종과 같은 칸 · 같은 선택 표시를 쓴다
- 투명은 **색이 아니다.** `Color.clear`로 바꾸지 않는다 — 고르던 색은 그대로 남아 있고
  다시 켜면 돌아온다 (`MirrorStyle.isFrameVisible`)
- 투명이면 **프레임 밴드 전체를 그리지 않는다.** paper grain도 같이 빠진다 —
  색만 빼고 결을 남기면 카메라 위에 점이 흩뿌려진 것처럼 보인다
- Editor에서는 스티커 캔버스와 **같은 체크무늬**로 "여기는 비어 있다"를 보여준다.
  새 표현을 만들지 않았다
- 장식(그림 · 스티커 · 사진 · 텍스트)은 그대로다. **프레임 없음 ≠ 장식 없음**
- geometry는 그대로다 — 1080 × 2340 · insets · 좌표 하나도 움직이지 않는다
- 잠금화면 Quick Mirror는 이미 있던 `.none` preset으로 간다. `transparent`를 새로 만들지 않았다
...
- **이 점선은 Editor에서만 보인다.** 실제 Mirror · Capture · 저장된 PNG ·
  잠금화면 Quick Mirror · 상점/홈 미리보기에는 절대 나오지 않는다.
  `showsCameraGuide`는 기본이 꺼짐이고 Editor canvas 한 곳에서만 켠다 —
  기본값이 production 화면에 새는 방향이면 안 된다
...
새 화면을 만들지 않았다. Sticker Creator 툴바에 버튼 하나가 늘어난다.

- 툴바: `AI` · 사진 · 그리기 · 스티커 · 텍스트 · 레이어
- **서버가 켜 주기 전에는 버튼 자체가 없다.** provider가 없는데 눌러서 실패
  대화상자를 보게 하지 않는다
- 프롬프트는 기존 bottom sheet 한 장(`AIStickerPromptSheet`)이다.
  Text 입력 sheet와 같은 모양 · 같은 글자 수 표시 · 같은 버튼 배치
- 가격은 sheet 안에 한 줄로만 보여준다: `6조각을 써요` / 모자라면
  `6조각이 필요해요 (지금 2조각)`. **숫자는 서버가 준 값이다**
- 만드는 동안은 기존 진행 overlay를 그대로 쓴다(사진 배경 제거와 같은 것).
  단계마다 문구만 다르다: `AI가 스티커를 만들고 있어요` → `스티커 배경을 정리하고 있어요`

### 끊겼을 때 (A-1B)

끊긴 생성이 있으면 **툴바의 `AI`가 `다시 확인`으로 바뀐다.** 새 배지 · 새 배너 ·
새 화면을 만들지 않는다 — 같은 자리에서 할 일만 바뀐다.

실패 대화상자도 기존 `inkDialog` 한 장이고, 서버에 작업이 남아 있을 때만
`다시 확인` · `그만두기`가 함께 나온다. 아니면 `확인` 하나다.
