...
## Shape / Decoration
- 툴바의 `장식` → 도형 고르기 → 고르면 **보고 있는 화면 한가운데**에 바로 생긴다.
- 종류 10개 — 기본(원 / 사각형 / 둥근 사각형 / 선) + 꾸미기(하트 / 별 / 물결선 / 리본 라인 / 테이프 / 말풍선).
- 상점 스티커 asset이 아니라 색과 크기를 자유롭게 바꾸는 Editor 자체 오브젝트다.
- 채우기 색 / 선 색 / 선 굵기 / 칠하기 방식(채우기·테두리·둘 다) / 투명도.
- 선 · 물결선 · 리본 라인은 선으로만 그린다 — 채우기를 무시한다.
- 이동 / 크기 / 회전 / 복제 / 잠금 / 삭제 / 완료 / 재선택은 스티커 · 텍스트와 같다.
- 크기는 비율을 유지한 채 커지고 작아진다 (한 방향만 늘리는 handle은 후속).
- 테이프 모양은 저장 후에도 변하지 않는다 — 렌더마다 난수를 쓰지 않는다.
- 말풍선은 모양만 제공한다. 안에 글자를 넣으려면 텍스트를 따로 올린다.
- 카메라 영역을 포함한 캔버스 어디에나 놓을 수 있다.
- 도형을 몇 개 넣든 거울 보관 슬롯은 하나다.

## Decoration 순서
- 스티커 / 사진 스티커 / 도형 / 텍스트는 **하나의 zIndex 세계**를 공유한다.
- 같은 zIndex면 스티커 → 도형 → 텍스트 순으로 위에 그려지고, tap 선택도 같은 규칙을 따른다.
- 새 장식은 언제나 가장 위에 올라간다.
- 사용자가 순서를 직접 바꾸는 Layers UI는 후속 Phase.
...
- **장식(그림 / 스티커 / 사진 / 도형 / 텍스트)**: 캔버스 전체에 그려진다. Camera Area 위도 포함이다.
- 실제 Mirror 레이어 순서: 카메라 → 프레임 배경 → 그림 → 템플릿 장식 → 스티커·도형·텍스트(zIndex 순).
