...
### 서버가 세지 않는 통계는 숫자로 말하지 않는다

**내장 템플릿(`StoreCatalog`)에는 서버 획득 통계가 없다.** 다운로드가 순수 로컬
동작(`MirrorLibrary.acquire`)이라 서버에 기록되는 곳도 세는 곳도 없다 — 감사로
확인했다. 그래서 `0`을 보여 주면 "아무도 안 받았다"는 **거짓말**이 된다.

`MirrorTemplate.hasServerStats`가 이것을 타입으로 표현하고, 거짓이면 카드·상세에서
통계 자리를 **비운다**. Marketplace 상품은 서버가 세므로 그대로 보여 준다.

내장 템플릿용 server-authoritative 획득 통계(`catalog` domain)는 **아직 만들지
않았다** — 별도 phase로 미뤘다.
